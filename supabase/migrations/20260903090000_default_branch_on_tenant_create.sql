...
-- created atomically inside the same transaction as the tenant itself.
-- Phase 2I.2's production smoke test discovered there was previously NO
-- application path anywhere that could create a branch — a real,
-- pre-existing, launch-blocking gap (staff/service eligibility and
-- appointments all require a branch to attach to). Default name
-- deliberately generic ("Merkez Şube" = central/main branch); no
-- address or phone invented — both stay null, matching the column's own
-- nullability (branches has no email column at all).
...
-- Backfill: any EXISTING tenant with zero ACTIVE branches (deleted_at is
-- null) gets exactly one Merkez Şube, same as a fresh signup now gets.
-- Tenants that already have one or more active branches are untouched.
-- In practice every existing tenant has zero branch rows at all (no path
-- could ever have created one), but this is written against "zero
-- active" rather than "zero rows at all" for correctness if that ever
-- changes.
insert into public.branches (tenant_id, name, is_primary)
select t.id, 'Merkez Şube', true
from public.tenants t
where not exists (
  select 1 from public.branches b where b.tenant_id = t.id and b.deleted_at is null
);
